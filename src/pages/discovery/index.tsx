...
const DiscoveryPage: React.FC = () => {
  ...
  const editorPickData = useMemo(() => {
    const result = discoveryData?.data?.find(
      (cell: any) => cell.type === "EDITOR_PICK"
    );
  ...
  const headerData = useMemo(() => {
    const result = discoveryData?.data?.find(
      (cell: any) => cell.type === "DISCOVERY_HEADER"
    );
  ...
  const recommendData = useMemo(() => {
    const result = discoveryData?.data?.find(
      (cell: any) => cell.type === "DISCOVERY_EPISODE_RECOMMEND"
    );
