...
import { api, request, timeToHours } from "@/utils/index";
...
const UserInfoPage: React.FC<{ uid: string; currentUser: boolean }> = (props: {
  uid: string;
  currentUser: boolean;
}) => {
...
const UserBasicInfo: React.FC<{ data: IUserBasicInfo }> = (props: {
  data: IUserBasicInfo;
}) => {
  return (
...
const UserFollowInfo: React.FC<{ data: IUserState; uid: string }> = (props: {
  data: IUserState;
  uid: string;
}) => {
  ...
  const jumpToPodcaster = (uid: string) => {
    navigate(`/overview/podcaster/${uid}`, { replace: true });
  };
...
const MileageRankInfo: React.FC<{ data: IMileAge }> = (props: {
  data: IMileAge;
}) => {
        ...
        <div className="flex items-center">
          <span>
            {displayHours.hours > 0 ? (
              <>
                <span className="text-3xl font-bold">{displayHours.hours}</span>
...
const PlayedHistoryList: React.FC<{ data: IEpisode[] }> = (props: {
  data: IEpisode[];
}) => {
  const navigate = useNavigate();
