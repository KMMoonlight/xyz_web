...
export default function Search() {
    ...
    const querySearchPreset = () => {
        api.apiGetSearchPreset().then((res) => {
    ...
    const scrollPreset = () => {
        ...
        interval = setInterval(() => {
            setSearchPresetValue((val) => {
                ...
                const firstValue = searchPresetData.length > 0 ? searchPresetData[0].text : '搜索'
                return index === -1 ? firstValue : (index + 1 >= searchPresetData.length) ? firstValue : searchPresetData[index + 1].text
            })
        }, 6000)
    }
    ...
    return (
        <div className="flex mt-4 w-full justify-center items-center">
            <div className="flex items-center relative">
                <Input value={searchValue} onInput={onSearchInput} placeholder={searchPresetValue} className="w-[640px] pl-[36px]" />
