...
const BuildIconWithTitle: React.FC<{
  title: string;
  alias: string;
  navigate?: boolean;
}> = React.memo(({ title, alias, navigate = false }) => {
  const useClass = navigate
  ...
  const icon =
    title === "home" ? (
      <img
        src={cosmos as string}
        style={{ width: iconSize, height: iconSize }}
        alt="cosmos"
      />
    ) : title === "subscription" ? (
      <Layers color="#25b4e1" size={iconSize} />
    ) : (
      <User color="#25b4e1" size={iconSize} />
    );
...
const NavigatorBar: React.FC<{ changeCurrentTab: (cell: string) => void }> =
  React.memo(({ changeCurrentTab }) => {
    return (
      <>
        {Object.keys(pageTitleMap).map((cell) => {
          return (
            <Link
              key={cell}
              to={`/overview/${cell}`}
              onClick={() => changeCurrentTab(cell)}
              className="mr-4 text-sm"
            >
              <div className="flex">
                <BuildIconWithTitle
                  title={cell}
                  alias={pageTitleMap[cell]}
                  navigate={true}
                />
              </div>
            </Link>
  ...
  return (
    <>
      <div className="w-full flex flex-col items-center justify-center relative">
        ...
        <div className="w-full h-[calc(100vh-220px)] flex flex-col items-center">
          <Outlet />
        </div>
        <PlayerPage />
      </div>
